...
public final class Route
{
    ...
    public static String searchRoute(String graphContent, String condition)
    {
        ...
        SortList<Solution> solutionSortList = new SortList<Solution>(new Comparator<Solution>() {
            @Override
        ...
        startSolution.func();
        solutionSortList.add(startSolution);

        Solution bestSolution = route.new Solution();
        bestSolution.sum = Integer.MAX_VALUE;

        while( solutionSortList.size() > 0 && ((System.currentTimeMillis() - startTime) <= 9 * 60 * 1000 )) {
            Solution currentSolution = solutionSortList.remove(0);
            ...
            for(int i = 0; i <= maxNodeId; i++) {
                if( (!route.byteContain(currentSolution.pastPoints, i)) && graph[currentSolution.point][i] != null) {
                    ...
                    newSolution.func();
                    solutionSortList.add(newSolution);
    ...
    class Solution {
        ...
        public double value;                        //该解的预测值
        public void func() {
            value = (double)sum * (double)noContainSize / countSize;
        }
